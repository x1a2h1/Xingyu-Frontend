 ...
 * 转换菜单数据为树形选择器数据
 */
export function transformMenuToTreeSelect(
  menus: Api.Menu.Tree[] | null,
  includeRoot = true,
): TreeDataNode[] {
  if (!menus) return [];

  const transformNode = (menu: Api.Menu.Tree): TreeDataNode => ({
    title: menu.name,
    value: menu.id,
    key: menu.id,
 ...
 * 转换菜单数据为树组件数据
 */
export function transformMenuToTree(
  menus: Api.Menu.Tree[] | null,
): Api.Menu.Tree[] {
  if (!menus) return [];
  return menus;
 ...
 * 生成菜单唯一标识
 */
export function generateMenuSymbol(
  name: string,
  parentSymbol?: string,
): string {
